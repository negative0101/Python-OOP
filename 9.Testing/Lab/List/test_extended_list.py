...
class TestIntegerList(TestCase):
    ...
    def test_init_takes_non_integers(self):
        self.list_integers = IntegerList(5.5, "6", 7.2)
        self.assertEqual([], self.list_integers._IntegerList__data)
    ...
    def test_get_with_invalid_index_raise_error(self):
        with self.assertRaises(IndexError) as ex:
            self.list_integers.get(3)
        self.assertEqual("Index is out of range", str(ex.exception))

    def test_insert_return_element(self):
    ...
    def test_insert_if_not_in_range(self):
        with self.assertRaises(IndexError) as ex:
            self.list_integers.insert(3, 1)
        self.assertEqual("Index is out of range", str(ex.exception))

    def test_get_biggest(self):
        biggest = self.list_integers.get_biggest()
        self.assertEqual(7, biggest)
